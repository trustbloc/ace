...
func TestNewClient(t *testing.T) {
	t.Run("URL parse error", func(t *testing.T) {
		client, err := vault.NewClient(
			"", "http://user^foo.com", nil, nil, testutil.DocumentLoader(t))
		require.Error(t, err)
...
func TestClient_CreateVault(t *testing.T) {
	...
	t.Run("Error parse zcap", func(t *testing.T) {
		...
		client, err := vault.NewClient(
			...
			store,
			loader,
			vault.WithRegistry(&vdr.MockVDRegistry{CreateValue: newDIDDoc()}),
		)
		require.NoError(t, err)
...
func createVaultID(t *testing.T, k vault.KeyManager) (string, string, string) {
	t.Helper()
