...
const (
	...
	kmsBaseURL        = "https://localhost:8074"
	kmsNetworkBaseURL = "https://kms.trustbloc.local:8074"
	edvBaseURL        = "https://localhost:8081/encrypted-data-vaults"
	edvNetworkBaseURL = "https://edv.trustbloc.local:8081/encrypted-data-vaults"
...
func (s *Steps) userCreatesProfile() error {
	...
	s.user, err = newUser(cshHost, edvBaseURL, kmsBaseURL, s.httpClient)
	if err != nil {
...
func verificationMethod(zcap *zcapld.Capability) string {
	return zcap.Proof[0]["verificationMethod"].(string) //nolint:forcetypeassert
}

func capabilityChain(zcap *zcapld.Capability) []interface{} {
	chain, ok := zcap.Proof[0]["capabilityChain"]
	if ok {
		return chain.([]interface{}) //nolint:forcetypeassert
	}
